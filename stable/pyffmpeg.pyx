...
IMAGE_LIBRARY_PIL="PIL"
IMAGE_LIBRARY_NUMPY="NumPy"
IMAGE_LIBRARY_CAIRO="cairo"
...
cdef class VideoStream:
    ...
    cdef AVFrame *frame
    cdef object image_library
    cdef int frameno
    cdef object filename
    cdef object index
    cdef object keyframes
    cdef object GetCurrentFrame
    cdef int pixformat
    ...
    def __cinit__(self):
        ...
        self.index = None
        self.keyframes = None
        self.pixformat=PIX_FMT_RGB24

    def dump(self):
        dump_format(self.FormatCtx,0,self.filename,0)

    def open(self,char *filename,image_library=IMAGE_LIBRARY_PIL):
    ...
    cdef AVFrame *ConvertToRGBA(self,AVPicture *frame,AVCodecContext *pCodecCtx):
        ...
        avpicture_fill(<AVPicture *>pFrameRGBA, rgb_buffer, PIX_FMT_RGB32,
                        width, height)

        convert_ctx = sws_getContext(width, height, pCodecCtx.pix_fmt, width,height,PIX_FMT_RGB32, SWS_BICUBIC, NULL, NULL, NULL)
        if convert_ctx == NULL:
    ...
    def __GetCurrentFrame_cairo(self):
        ...
        width = self.CodecCtx.width
        height = self.CodecCtx.height
        pFrame = self.ConvertToRGBA(<AVPicture *>self.frame, self.CodecCtx)
        numBytes = avpicture_get_size(PIX_FMT_RGB32, width, height)
    ...
    cdef AVFrame *_convert_withbuf(self,AVPicture *frame,char *buf,  int pixformat=-1):
        ...
        width = self.CodecCtx.width
        height = self.CodecCtx.height
        avpicture_fill(<AVPicture *>pFramePixFormat, <uint8_t *>buf, pixformat,   width, height)

        convert_ctx = sws_getContext(width, height, pCodecCtx.pix_fmt, width,height,pixformat, SWS_BICUBIC, NULL, NULL, NULL)
        if convert_ctx == NULL:
            raise MemoryError("Unable to allocate scaler context")

        sws_scale(convert_ctx, frame.data, frame.linesize, 0,  self.CodecCtx.height, <uint8_t**>pFramePixFormat.data, pFramePixFormat.linesize)

        sws_freeContext(convert_ctx)
        return pFramePixFormat

    def __GetCurrentFrame_numpy(self,pixformat=None):
        cdef AVFrame * pFrameRes
        if (pixformat==None):
          pixformat=self.pixformat
        numBytes=avpicture_get_size(pixformat,  self.CodecCtx.width,  self.CodecCtx.height)
        img_image=numpy.ndarray(shape=(self.CodecCtx.height,self.CodecCtx.width,numBytes/( self.CodecCtx.width* self.CodecCtx.height)),dtype=numpy.uint8)
        pFrameRes = self._convert_withbuf(<AVPicture *>self.frame,<char *><unsigned long long>pyffmpeg_numpybindings.PyArray_DATA_content(img_image))
        av_free(pFrameRes)
        return img_image

    def SetImageLibrary(self,image_library,pixformat=None):
        if image_library == IMAGE_LIBRARY_PIL:
            self.image_library=image_library
            self.GetCurrentFrame=self.__GetCurrentFrame_PIL
        elif image_library == IMAGE_LIBRARY_CAIRO:
            self.image_library=image_library
            self.GetCurrentFrame=self.__GetCurrentFrame_cairo
        elif image_library == IMAGE_LIBRARY_NUMPY:
            self.image_library=image_library
            self.GetCurrentFrame=self.__GetCurrentFrame_numpy
            if (pixformat):
